...
/* tgeneric usage:
 ...
 tgeneric_ccs.c in your test file.
...
static void
rounding_failed (mpc_srcptr op, long int op2, mpc_srcptr rop,
  ...
  for (prec = prec_min; prec <= prec_max; prec+=step)
    {
      ...
      } while (!mpfr_fits_slong_p (MPC_RE (x), GMP_RNDN));
      y = mpfr_get_si (MPC_RE(x), GMP_RNDN);
