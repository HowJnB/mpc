...
/* tgeneric usage:
 1. use tgeneric_cc.c with function whose prototype is
 ...
 tgeneric_cc.c in your test file.
...
static void
rounding_failed (mpc_srcptr op, mpc_srcptr rop,
...
static void
reuse_failed (mpc_t z, mpc_t expected, mpc_t got)
{
  printf ("Error for %s(z, z) for ", FUNCTION_NAME(TEST_FUNCTION));
  OUT (z);
  OUT (expected);
  OUT (got);
