...
/* tgeneric usage:
 1. use tgeneric_ccu.c with function whose prototype is
...
static void
rounding_failed (mpc_srcptr op, unsigned long int op2, mpc_srcptr rop,
  ...
  for (prec = prec_min; prec <= prec_max; prec+=step)
    {
      ...
      do {
        test_default_random (x, 0, 32, 1);
      } while (!mpfr_fits_ulong_p (MPC_RE (x), GMP_RNDN));
      y = mpfr_get_ui (MPC_RE(x), GMP_RNDN);
      ...
      yy = y;
      TEST_FUNCTION (z, x, yy, MPC_RNDNN);
      TEST_FUNCTION (x, x, yy, MPC_RNDNN);
      if (mpc_cmp (z, x) != 0)
        {
          mpc_t expected;
          mpc_t got;
