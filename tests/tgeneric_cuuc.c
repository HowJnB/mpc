/* File for generic tests associated with complex function of first and second
   parameters with type unsigned long int, and third with type mpc_t.
...
/* tgeneric usage:
 1. use tgeneric_cuuc.c with function whose prototype is
 (mpc_t rop, unsigned long int op1,  unsigned long int op2, mpc_t op3, mpc_rnd_t rnd)
...
static void
rounding_failed (unsigned long int op1, unsigned long int op2, mpc_srcptr op3,
                 mpc_srcptr rop, mpc_srcptr rop4, mpc_srcptr rop4rnd,
                 mpc_rnd_t rnd)
{
  ...
  for (prec = prec_min; prec <= prec_max; prec+=step)
    {
      ...
      do {
        test_default_random (z, 0, 32, 1);
      } while (!mpfr_fits_ulong_p (MPC_RE (z), GMP_RNDN)
               || !mpfr_fits_ulong_p (MPC_IM (z), GMP_RNDN));
      x = mpfr_get_ui (MPC_RE (z), GMP_RNDN);
      y = mpfr_get_ui (MPC_IM (z), GMP_RNDN);
      ...
      for (rnd_re = 0; rnd_re < 4; rnd_re ++)
	    ...
	    TEST_FUNCTION (u, x, y, z, rnd);
	    TEST_FUNCTION (v, x, y, z, rnd);
            ...
            if (mpc_cmp (v, t) != 0)
              {
                ...
                printf ("%s(x,y,z) and %s(x,y,z) differ for rnd=(%s,%s)\n",
      ...
      TEST_FUNCTION (v, x, y, z, MPC_RNDNN);
      TEST_FUNCTION (z, x, y, z, MPC_RNDNN);
      if (mpc_cmp (z, v) != 0)
        {
          mpc_t expected;
          mpc_t got;
          ...
          printf ("Error for %s(t, x, y, t) for ",
                  FUNCTION_NAME(TEST_FUNCTION));
          printf ("x=%lu\n", x);
          printf ("y=%lu\n", y);
          OUT (t);
