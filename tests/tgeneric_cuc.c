/* File for generic tests associated with complex function of first parameter
   with type unsigned long int and second with type mpc_t.
...
/* tgeneric usage:
 1. use tgeneric_cuc.c with function whose prototype is
 ...
 tgeneric_cuc.c in your test file.
...
static void
rounding_failed (unsigned long int op, mpc_srcptr op2, mpc_srcptr rop,
  ...
  for (prec = prec_min; prec <= prec_max; prec+=step)
    {
      ...
      do {
        test_default_random (y, 0, 32, 1);
      } while (!mpfr_fits_ulong_p (MPC_RE (y), GMP_RNDN));
      x = mpfr_get_ui (MPC_RE(y), GMP_RNDN);
