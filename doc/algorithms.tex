...
To show that no more subcases lead to dyadic results, assume that $f \leq -3$
and let $x^y = \alpha \zeta = \alpha \zeta_r + i \alpha \zeta_i$, where
$\alpha = (-x)^y \in \R$ and $\zeta = \zeta_r + i \zeta_i$ is a primitive root
of unity of order~$2^{|f| + 1}$.
If both parts of $x^y$, $\alpha \zeta_r$ and $\alpha \zeta_i$, are elements
of $\Q (i)$, then $\zeta \in \Q (i, \alpha)$. Moreover,
...
How to exclude that only one part is dyadic?
\item
\item
\item
\end {enumerate}
